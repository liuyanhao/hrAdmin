...
public class ResumeController extends BaseController {
	...
	public ModelAndView exportExcel() throws Exception{
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var26", varOList.get(i).getString("CREATE_TIME"));	    //26
			vpd.put("var27", varOList.get(i).getString("UPDATE_TIME"));	    //27
			vpd.put("var28", varOList.get(i).getString("CREATE_USER"));	    //28
			vpd.put("var29", varOList.get(i).getString("UPDATE_USER"));	    //29
			vpd.put("var30", varOList.get(i).getString("STAFF_NAME"));	    //30
			vpd.put("var31", varOList.get(i).get("STATUS").toString());	//31
			vpd.put("var32", varOList.get(i).getString("USER_NAME"));	    //32
			vpd.put("var33", varOList.get(i).getString("REASON"));	    //33
			vpd.put("var34", varOList.get(i).get("RESULT").toString());	//34
			vpd.put("var35", varOList.get(i).getString("SCORE"));	    //35
			vpd.put("var36", varOList.get(i).get("JOBMESSAGE_ID").toString());	//36
			vpd.put("var37", varOList.get(i).getString("DEGREE"));	    //37
			varList.add(vpd);
		}
